...
public class CameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Todo change if when levitation behaviour is chosen
        if (Input.GetMouseButtonDown(1) && !LevitateBehaviour.IsLevitating && !FindObjectOfType<LevitateBehaviourStaticLevitation>())
        {
        ...
        {
            // Todo change when levitationbehaviour is chosen
            if (!LevitateBehaviour.IsRotating && !LevitateBehaviourStaticLevitation.IsRotating)
            {
    ...
    {
        // Todo change when levitationbehaviour is chosen
        if (LevitateBehaviour.IsRotating || LevitateBehaviourStaticLevitation.IsRotating) { return; }
